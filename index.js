...
const start = async (bot) => {
    ...
    bot.onMessage(async (message) => {
        ...
        if (message.body === "#hi" || message.body === "#hai" || message.body === "#halo" || message.body === "#hello") {
            console.log(`[DEBUG] [runCmd] ${moment.tz("Asia/Jakarta").format()} => Someone has used 'hi' command: ${message.from} ( ${message.sender.pushname} | ${message.chat.name} )`);
        ...
        if (message.type === "image" && message.caption === "#sticker") {
            console.log(`[DEBUG] [create] ${moment.tz("Asia/Jakarta").format()} => Someone has just created a new sticker by attaching: ${message.from} ( ${message.sender.pushname} | ${message.chat.name} )`);
        ...
        if (message.quotedMsg && message.quotedMsg.type === "image" && message.body === "#sticker") {
            console.log(`[DEBUG] [create] ${moment.tz("Asia/Jakarta").format()} => Someone has just created a new sticker by quoting: ${message.from} ( ${message.sender.pushname} | ${message.chat.name} )`);
